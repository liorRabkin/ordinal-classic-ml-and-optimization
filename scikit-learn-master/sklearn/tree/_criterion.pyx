...
############################################################################
#############################################################################
cdef class WIGR_EV_fix(ClassificationCriterion):
    ...
    cdef double node_impurity(self) nogil:
        ...
        for k in range(self.n_outputs):
            for c in range(n_classes[k]):
                ...
                self.EV_fix += count_k * self.y_values[c] 
    ...
    cdef void children_impurity(self, double* impurity_left,
        ...
        for k in range(self.n_outputs):
            ...
            for c in range(n_classes[k]):
                ...
                sum_v_left += fabs(self.y_values[c]- self.EV_fix)**self.WIGR_power 
                sum_v_right += fabs(self.y_values[c]- self.EV_fix)**self.WIGR_power 
                    
            for c in range(n_classes[k]):

                k_i_left = (fabs(self.y_values[c] - self.EV_fix)**self.WIGR_power) / sum_v_left # need to add power 
                k_i_right = (fabs(self.y_values[c] - self.EV_fix)**self.WIGR_power) / sum_v_right # need to add power 
